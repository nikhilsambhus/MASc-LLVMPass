...
namespace {
  struct SkeletonPass : public FunctionPass {
	...
	Loop *lin;
	std::unique_ptr<LoopNest> lnest = LoopNest::getLoopNest(*L, SE); 
	if(lnest->getNumLoops() > 1) {
		lin = lnest->getInnermostLoop();
	}
	else {
		lin = L;
	}
	...
	PHINode *phinode = lin->getInductionVariable(SE);
...
static RegisterPass<SkeletonPass> X("sr", "Strength Reduction Pass");
